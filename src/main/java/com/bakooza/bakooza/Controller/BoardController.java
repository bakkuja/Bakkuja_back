...
//            }
            ...
            BoardDTO boardDTO = new BoardDTO(params.getTitle(), params.getUserLocation(), params.getCategoryId(), params.getContent(), writer, memberId, null);
            //postId = boardService.save(boardDTO);
            postId = awsS3Service.uploadFile(multipartFile, boardService.save(boardDTO));
            boardService.updateThumbnail(postId, postImageRepository.findByPostId(postId).get(0).getImagePath());
...
//            }
    ...
    // 게시글 수정
    @PatchMapping("/{postId}")
    public ResponseEntity<Object> update(@PathVariable final Long postId, @RequestPart(value="accessToken") TokenDTO accessToken, @RequestPart(value="params") BoardRequestDTO params, @RequestPart(value="multipartFile", required = false) MultipartFile multipartFile) {
...
//            }
            ...
            List<ImageResponseDTO> entity = boardService.findByPostId(postId); // S3의 이미지 경로를 찾아오고
    ...
    //게시글 검색
    @GetMapping("/search")
    public ResponseEntity<Object> search(@RequestParam final String keyword) {
        return new ResponseEntity<>(boardService.search(keyword), HttpStatus.OK);
    }
...
//        return new ResponseEntity<>(boardService.findByCategoryId(categoryId, pageable).map(BoardResponseDTO::new), HttpStatus.OK);
        return new ResponseEntity<>(boardService.findByCategoryId(categoryId), HttpStatus.OK);
    }

    // 게시판 전체 조회
    @GetMapping("/items")
    public ResponseEntity<Object> findAll() {
        return new ResponseEntity<>(boardService.findAllPostId().stream().map(BoardResponseDTO::new).collect(Collectors.toList()), HttpStatus.OK);
    }
